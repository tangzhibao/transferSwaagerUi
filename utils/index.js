...
function createApiAnnotation({ paramsIn, type, name, description }, data, action) {
...
function parametersParser(pathsData) {
  return Object.entries({...pathsData}).reduce(
    (r, [path, pathInfo]) => r.concat(
      Object.entries(pathInfo).map(
        ([method, apiDetail]) => ({ ...apiDetail, method, path })
      )
    )
  , [])
}
    ...
    for (const [k, v] of Object.entries(obj)) {
      if (firstTime) {
        ...
        if ('type' in v && !(['object', 'array'].includes(v.type))) {
          ...
          if (continueMap) originalRef = v.type === 'array'
            ? v.items.originalRef
            : v.originalRef
          ...
          param = {
            ...
            items: continueMap
              ...
              : {
                ...
                description: (v.type === 'array' ? v.items.description : v.description) || '',
              },
          }
        }
        tar[k] = param
        if (originalRef !== undefined) stack.push([
          defData[originalRef].properties,
          tar[k].items,
        ])
      }
    }
