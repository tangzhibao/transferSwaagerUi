const { judgeType, filterType, firstWordUppderCase, createApiAnnotation } = require('./index')
...
class Pipe {
  ...
  flatContent = action => {
    const data = Object.entries(this[action]).reduce((r, [k, v]) => {
      if ('schema' in v) {
        ...
        if (!('$ref' in v.schema)) r[k].items = { ...v.schema }; delete r[k].type
      }
      else {
        r[k] = { ...v }
      }
  ...
  sortPageFiled = (pageFields = []) => {
    this.req = this.req
      ...
      .map(item => {
        switch(item.name) {
          case 'pageNumber': return { ...item, name: 'page', description: '查询页码' };
          case 'pageSize': return { ...item, name: 'size', description: '分页大小' };
          default: return { ...item };
        }
      })
    return this    
